...
public class DeployProcessDefinitionCommandHandler implements CommandHandler<DeployProcessDefinitionCommand, ResponseEntity<ProjectResponseDTO>> {
  ...
  private final ProcessDeploymentPort processDeploymentPort;
   ...
   public DeployProcessDefinitionCommandHandler(ProjectRepository projectRepository, ProjectMapper projectMapper, ProcessDeploymentPort processDeploymentPort) {
   ...
   public ResponseEntity<ProjectResponseDTO> handle(DeployProcessDefinitionCommand command) {
     ...
     try (InputStream inputStream = file.getInputStream()) {
       ...
       for (Process process : processes) {
         ...
         draftProcessDefinition.orElseThrow(() ->
             IgrpResponseStatusException.badRequest("Process definition with key '" + processKey + "' not found in draft state."));


           var processDefinition = draftProcessDefinition.get();
